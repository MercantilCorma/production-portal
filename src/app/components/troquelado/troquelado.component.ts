...
@Component({
  selector: 'app-troquelado',
  templateUrl: './troquelado.component.html',
  styleUrls: ['./troquelado.component.css']
})
export class TroqueladoComponent implements OnInit {
  now!: Date;
  ...
  Post(){
    this.operations.postbitroquelado(this.miForm.value)
    .subscribe(
      res=> {
        console.log(res);
      });
    ...
    get(){
      this.operations.getbitroquelado(this.miForm.value['ordenpro'])
        .subscribe((res) => {
    ...
    delateinfo(id_bittroqueladora:number){
      if(confirm('¿Seguro que quieres Eliminarlo?')){
      this.operations.delatebitroquelado(id_bittroqueladora)
      .subscribe(
        res => {
          this.get();
        },err => {
