...
@Component({
  selector: 'app-suaje',
  templateUrl: './suaje.component.html',
  styleUrls: ['./suaje.component.css']
})
export class SuajeComponent implements OnInit {
  now!: Date;
  dia: Date = new Date();
  ...
  Postbitsuaje(){
    this.clientesServices.postbitsuaje(this.miForm.value)
    .subscribe(
      res=> {
        console.log(res);
      });
    ...
    getinfosuaje(){
      this.clientesServices.getbitsuaje(this.miForm.value['ordenpro'],this.miForm.value['fecha'])
        .subscribe((res) => {
    ...
    delateinfo(id_suaje:number){
      ...
      this.clientesServices.delateinfoOrdersSuaje(id_suaje)
      .subscribe(
        res => {
          this.getinfosuaje();
        },err => {
