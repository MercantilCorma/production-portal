...
miForm: FormGroup = this.fb.group({
  ...
  Postbitcorte(){
    this.clientesServices.postbitcorte(this.miForm.value)
    .subscribe(
      res=> {
        console.log(res);
      });
    ...
    getinfocorte(){
      this.clientesServices.getbitcorte(this.miForm.value['ordenpro'],this.miForm.value['fecha'])
        .subscribe((res) => {
   ...
   bit2(){
    ...
    delateinfo(id_corte:number){
      ...
      this.clientesServices.delateinfoOrdersCorte(id_corte)
      .subscribe(
        res => {
          this.getinfocorte();
        },err => {
